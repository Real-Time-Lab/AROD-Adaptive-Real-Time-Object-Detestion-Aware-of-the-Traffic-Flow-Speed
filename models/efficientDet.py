...
def main():
    root = root = os.path.join(os.environ['HOME'],'./Documents/datasets/traffic')  # ubuntu
...
def measure_model_size():
    ...
    result:
        Flops: 20273231104.0, Parameters:22894902.0
        GFLOPs: 20.27
    ...
    model.load_state_dict(torch.load(f'../../efficientDet/weights/efficientdet-d{0}.pth'))
    model.requires_grad_(False)
    model.eval()

    flops, params = profile(model.to(device), inputs=(torch.rand(1,3, 512, 512).to(device),))
